...
foodlengthtrials = datatable(1:5:end);
foodwidthtrials = datatable(2:5:end);
indexdatatrials = datatable(3:5:end);
Mgatrials = datatable(4:5:end);
thumbdatatrials = datatable(5:5:end);
x = table(Mgatrials,thumbdatatrials,indexdatatrials,foodwidthtrials,foodlengthtrials);

%------------STRIP VELOCITY, X/Y POSN and ACCEL data------------%
%STRIP thumb data
for i = 1:height(x) 
x.thumbvelocity{i,1} = x.thumbdatatrials{i,1}(:,[1,4]);%Velocity
x.thumbaccel{i,1} = x.thumbdatatrials{i,1}(:,[1,5]); %Accel
x.thumbXYposn{i,1} = x.thumbdatatrials{i,1}(:,[2,3]); %X/Y Posn
%Strip index data
x.indexvelocity{i,1} = x.indexdatatrials{i,1}(:,[1,4]);%Velocity
x.indexaccel{i,1} = x.indexdatatrials{i,1}(:,[1,5]); %Accel
x.indexXYposn{i,1} = x.indexdatatrials{i,1}(:,[2,3]); %X/Y Posn
end
x.thumbdatatrials = [];
x.indexdatatrials = [];
